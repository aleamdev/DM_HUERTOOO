...
fun CatalogScreen(
    ...
    Scaffold(
        ...
        LazyVerticalGrid(
            ...
            items(filtered) { p -> ProductCard(p, onAdd = { /* TODO carrito */ }) { goToDetail(p.id) } }
        }
    }
}
...
private fun ProductCard(p: Product, onAdd: () -> Unit, onClick: () -> Unit) {
    Card(onClick = onClick) {
        Column(Modifier.padding(12.dp)) {
            AsyncImage(model = p.imageUrl, contentDescription = null, modifier = Modifier.fillMaxWidth().height(120.dp))
            Spacer(Modifier.height(8.dp))
            Text("${p.sku} - ${p.name}", style = MaterialTheme.typography.titleSmall)
            if (p.priceText.isNotBlank()) Text("Precio: ${p.priceText}", style = MaterialTheme.typography.bodyMedium)
            p.description?.let { Text(it, style = MaterialTheme.typography.bodySmall, maxLines = 3) }
            Spacer(Modifier.height(8.dp))
